...
const _edgeUrl =
    'https://sakeuhcbcnueplzlkltm.supabase.co/functions/v1/license-manager';
const _anonKey =
...
class ActivationRecord {
  ...
  factory ActivationRecord.fromJson(Map<String, dynamic> j) =>
      ActivationRecord(
        id: j['id'] as String,
...
class LicenseDetail extends LicenseRecord {
  ...
  factory LicenseDetail.fromJson(Map<String, dynamic> j) {
    ...
    return LicenseDetail(
      ...
      googleUserId: j['google_user_id'] as String?,
      expiresAt: j['expires_at'] as String?,
      activationCount: (j['activation_count'] as num?)?.toInt() ??
          (j['device_count'] as num?)?.toInt() ??
          acts.length,
      activations: acts,
    );
  }
...
class LicenseStats {
  final int total, generated, trial, active, cancelled, expired, suspended,
      totalActivations;
  LicenseStats({
    required this.total,
...
class LicenseListResponse {
  ...
  LicenseListResponse(
      ...
      required this.page,
      required this.limit});
  factory LicenseListResponse.fromJson(Map<String, dynamic> j) =>
      LicenseListResponse(
        licenses: (j['licenses'] as List<dynamic>)
            .map((l) => LicenseRecord.fromJson(l as Map<String, dynamic>))
            .toList(),
        total: (j['total'] as num?)?.toInt() ?? 0,
...
class AdminRepository {
  ...
  static Future<Map<String, dynamic>> _call(
    ...
    final res = await http.post(
      ...
      body: jsonEncode({...params, 'action': action}),
    );

    final data = jsonDecode(res.body) as Map<String, dynamic>;
    if (res.statusCode >= 400) {
      throw Exception(data['error'] as String? ?? 'HTTP ${res.statusCode}');
    }
    return data;
  }
  ...
  static Future<LicenseListResponse> listLicenses(
      {int page = 0, int limit = 30, String? status, String? search}) async {
  ...
  static Future<Map<String, dynamic>> addKey(
      {required String key,
      required String serial,
      String? ownerEmail}) async {
    return _call('add',
        {'key': key, 'serial': serial, 'owner_email': ownerEmail ?? ''});
  }
